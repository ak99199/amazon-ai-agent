...
class AdsReportTransport:
    ...
    def poll(self,profile_id,report_id):
        for attempt in range(self.max_attempts):
            ...
            normalized={"success":"completed","completed":"completed","failure":"failed","failed":"failed","cancelled":"cancelled","processing":"processing","requested":"requested"}.get(status,"unknown")
            result=AdsLiveReportStatus(report_id,normalized,payload.get("url") or payload.get("location") if isinstance(payload,dict) else None)
