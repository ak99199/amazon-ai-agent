...
class DashboardSecurityMiddleware(BaseHTTPMiddleware):
    async def dispatch(self,request,call_next):
        path=request.url.path; protected=path.startswith("/dashboard") or path.startswith("/api/listings") or path.startswith("/api/portfolio") or path.startswith("/api/internal") or path.startswith("/api/alerts") or path.startswith("/api/ads")
        ...
        if protected:
            ...
            if request.method in ("POST","PUT","PATCH","DELETE") and (path.startswith("/api/internal") or path.startswith("/api/alerts") or path.startswith("/api/ads")):
