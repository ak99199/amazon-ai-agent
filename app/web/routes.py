...
def _normalize_ads_readiness(value):
 ...
 live=result.get("production_live_read")
 required=("approval_status","credential_configuration_complete","profile_selected","live_read_enabled","mock_mode","region","blocking_reasons","manual_smoke_test_allowed")
 if not isinstance(live,dict) or not all(field in live for field in required):
  result["production_live_read"]={"unavailable":True,"approval_status":"unknown","credential_configuration_complete":False,"profile_selected":False,"live_read_enabled":False,"mock_mode":True,"region":None,"blocking_reasons":[],"manual_smoke_test_allowed":False}
 else:
  result["production_live_read"]={**live,"unavailable":bool(live.get("unavailable",False))}
 return result
