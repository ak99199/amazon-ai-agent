...
from app.amazon_ads.config import ADS_REGION_ENDPOINTS
...
class AdsLiveReadService:
    ...
    def status(self,require_profile=True):
        ...
        if self.settings.region not in ADS_REGION_ENDPOINTS:
            return LiveReadStatus("blocked_config",True,False,approval,True,profile,False,"region_invalid")
        if require_profile and not profile:
            return LiveReadStatus("blocked_profile",True,False,approval,True,False,False,"profile_not_selected")
        return LiveReadStatus("ready_live",True,False,approval,True,profile,True)
    def _ready(self,require_profile=True):
        state=self.status(require_profile=require_profile)
        if not state.ready:raise AdsLiveReadBlockedError(state.mode)
    def discover_profiles(self):
        self._ready(require_profile=False)
        profiles=[]
