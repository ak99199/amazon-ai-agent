...
class AdsManualHistoricalSyncService:
 def __init__(self,readiness_service,gate_service,repository,persistence_service,now,recovery_service=None):self.readiness=readiness_service;self.gate=gate_service;self.repository=repository;self.persistence=persistence_service;self.now=now;self.recovery=recovery_service;self.execution=AdsHistoricalSyncExecutionService(repository,persistence_service,now)
