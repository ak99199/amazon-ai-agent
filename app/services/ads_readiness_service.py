...
class AdsReadinessResult:
    approval_status:str;config_status:str;profile_status:str;data_status:str;overall_status:str;has_client_id:bool;has_client_secret:bool;has_refresh_token:bool;has_profile_id:bool;has_historical_data:bool;performance_row_count:int;ingestion_run_count:int;last_ingestion_at:str|None;last_successful_ingestion_at:str|None;approval_message:str;config_message:str;profile_message:str;data_message:str
...
class AdsReadinessService:
    ...
    def get(self,seller_id,marketplace_id):
        ...
        return AdsReadinessResult(approval,"complete" if has_id and has_secret and has_refresh else "incomplete","selected" if has_profile else "not_selected","available" if has_data else "no_data",overall,has_id,has_secret,has_refresh,has_profile,has_data,diagnostics["performance_row_count"],diagnostics["ingestion_run_count"],diagnostics["last_ingestion_at"],diagnostics["last_successful_ingestion_at"],f"Amazon Ads approval is {approval}.","Ads credentials are complete." if has_id and has_secret and has_refresh else "Ads credentials are incomplete.","Ads profile is selected." if has_profile else "Ads profile is not selected.","Historical Ads data is available." if has_data else "No historical Ads data is available.")