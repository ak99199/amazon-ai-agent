...
class AdsScheduledHistoricalSyncService:
 ...
 def run(self,seller_id,marketplace_id):
  ...
  if previous:
   anchor=previous.finished_at or previous.started_at
   if anchor+timedelta(hours=self.config.interval_hours)>now:return self._result("not_due",None,None,0,"Scheduled historical sync is not due yet.")
