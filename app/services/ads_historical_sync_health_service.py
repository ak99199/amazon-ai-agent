...
class AdsHistoricalSyncHealthService:
 ...
 def get(self,seller_id,marketplace_id,profile_id=None,limit=10):
  now=self.now();profile_id=profile_id or self.gate.settings.profile_id;runs=self.repository.list_sync_runs(seller_id,marketplace_id,profile_id,max(1,min(limit,20)),HISTORICAL_SYNC_MODE);latest=runs[0] if runs else None;success=self.repository.latest_successful_sync(seller_id,marketplace_id,profile_id,HISTORICAL_SYNC_MODE);active=bool(self.repository.active_sync_run(seller_id,marketplace_id,profile_id));start=now.date()-timedelta(days=2);end=now.date()-timedelta(days=1);gate=self.gate.evaluate(seller_id,marketplace_id,profile_id,start,end,7,HISTORICAL_SYNC_MODE);last_date=self.repository.latest_campaign_performance_date(seller_id,marketplace_id,profile_id)
  ...
  latest_success=self.repository.latest_successful_sync(seller_id,marketplace_id,profile_id,HISTORICAL_SYNC_MODE);remaining=0
