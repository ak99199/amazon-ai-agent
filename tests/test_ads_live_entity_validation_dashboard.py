...
def page(monkeypatch,ready):
 configure_admin(monkeypatch);monkeypatch.setenv("AMAZON_SELLER_ID","seller");monkeypatch.setenv("AMAZON_MARKETPLACE_ID","market");state={"approval_status":"approved" if ready else "pending","config_status":"complete" if ready else "incomplete","profile_status":"selected" if ready else "not_selected","data_status":"no_data","ingestion_run_count":0,"last_ingestion_at":None,"overall_status":"ready" if ready else "approval_pending","production_live_read":production(ready)};monkeypatch.setattr(routes,"_ads_readiness",lambda context:state);client=TestClient(app);login(client);return client.get("/dashboard")

def test_validation_button_only_when_ready_and_safety_copy(monkeypatch):
 blocked=page(monkeypatch,False);assert blocked.status_code==200 and "Validate Profile &amp; Campaign Read" not in blocked.text
 ready=page(monkeypatch,True);assert "Validate Profile &amp; Campaign Read" in ready.text and "does not modify campaign state, budgets, bids, keywords, or targeting" in ready.text

def test_dashboard_readiness_failure_hides_validation(monkeypatch):
 configure_admin(monkeypatch);monkeypatch.setenv("AMAZON_SELLER_ID","seller");monkeypatch.setenv("AMAZON_MARKETPLACE_ID","market");monkeypatch.setattr(routes,"_ads_readiness",lambda context:{"unavailable":True});client=TestClient(app);login(client);response=client.get("/dashboard");assert response.status_code==200 and "Validate Profile &amp; Campaign Read" not in response.text
