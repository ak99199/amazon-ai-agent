...
def test_report_requests_support_future_levels():
    ...
    assert "unitsSoldClicks14d" in value.build_request("campaign",date(2026,1,1),date(2026,1,2)).columns
def test_normalizes_optional_dimensions_and_numbers():
    ...
    assert value.keyword_id is None and value.search_term is None and value.impressions==100 and value.spend==Decimal("12.34") and value.sales==Decimal("50") and value.units==2
    assert service().normalize_row("seller","market","profile",row(unitsSoldClicks14d=3)).units==3
